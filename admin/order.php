...
Transaction Details: <br>'.$row['bank_transaction_info'];
            ...
            $order_detail .= '
Customer Name: '.$row['customer_FirstName'].' '.$row['customer_LastName'].'<br>
...
';
        ...
        sendEmailAdmin($_POST['cust_first_name'],$_POST['cust_email'],$message_text,$subject_text);
        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM tbl_payment ORDER by pay_id DESC");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr class="<?php if($row['payment_status']=='Pending'){echo 'bg-r';}else{echo 'bg-g';} ?>">
                        ...
                        <td>
                           <?php
                           $statement1 = $pdo->prepare("SELECT * FROM tbl_order t1
                           JOIN company t2
                                    ON t1.company_id = t2.company_id WHERE payment_id=?");
                           $statement1->execute(array($row['payment_id']));
                           $result1 = $statement1->fetchAll(PDO::FETCH_ASSOC);
                           foreach ($result1 as $row1) {
                                echo '<b>Product:</b> '.$row1['product_name'];
                                echo '</br>';
                                echo '<b>Company:</b> '.$row1['company_name'];
