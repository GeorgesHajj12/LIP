...
foreach ($result as $row) {
	...
	if($time>24) {
		...
		foreach ($result1 as $row1) {
			$statement2 = $pdo->prepare("SELECT * FROM products WHERE p_id=?");
		...
		$statement1 = $pdo->prepare("DELETE FROM tbl_payment WHERE pay_id=?");
		$statement1->execute(array($row['pay_id']));
	}
}
?>
<!DOCTYPE html>
...
<head>
	...
	{
		$statement = $pdo->prepare("SELECT * FROM products WHERE p_id=?");
		...
		<div class="row inner">
			...
			<div class="col-md-5 right">
				<ul>
					...
					if(isset($_SESSION['customer'])) {
						?>
						<li><i class="fa fa-user"></i> <?php echo 'Logged in as'; ?> <?php echo $_SESSION['customer']['cust_FirstName'].' '.$_SESSION['customer']['cust_LastName'] ; ?></li>
			...
			<div class="col-md-3 search-area">
				<form class="navbar-form navbar-left" role="search" action="search-result.php" method="get">
					...
					<button type="submit" class="btn btn-danger"><?php echo 'Submit'; ?></button>
				</form>
			</div>
		</div>
	</div>
