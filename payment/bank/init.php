...
if( !isset($_REQUEST['msg']) ) {
	if(empty($_POST['transaction_info'])) {
	    ...
	    foreach($_SESSION['cart_p_id'] as $key => $value) 
	    {
			...
			$row = $pdo->query("SELECT company_id from products where p_id= $arr_cart_p_id[$i]")->fetch();
		...
		foreach($_SESSION['cart_p_id'] as $key => $value) 
	    {
	        $i++;
			$row = $pdo->query("SELECT company_id from products where p_id= $arr_cart_p_id[$i]")->fetch();
	        $arr_cart_p_comp_id[$i] = $row['company_id'];
		...
		$body='Your order is sent, thank u!';
		sendEmailKK($_SESSION['customer']['cust_FirstName'],$_SESSION['customer']['cust_email'],$body);
