...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
        ...
        $order_detail = '';
        $statement = $pdo->prepare("SELECT * FROM tbl_payment t1
        JOIN tbl_order t2
        ON t1.payment_id = t2.payment_id
       WHERE payment_id=? AND company_id=?");
        $statement->execute(array($_POST['payment_id'],$_SESSION['company']['company_id']));
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
...
Payment Id: '.$row['payment_id'].'<br>
        ...
        $i=0;
        $statement = $pdo->prepare("SELECT * FROM tbl_order WHERE payment_id=? AND company_id=?");
        $statement->execute(array($_POST['payment_id'],$_SESSION['company']['company_id']));
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
            $i++;
        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
                ...
                JOIN tbl_order t2
                ON t1.payment_id = t2.payment_id
               WHERE company_id=? ORDER by t1.pay_id DESC");
            	...
            	foreach ($result as $row) {
					...
					<tr class="<?php if($row['payment_status']=='Pending'){echo 'bg-r';}else{echo 'bg-g';} ?>">
	                    ...
	                    <td>
                            ...
                            <b>Name:</b><br> <?php echo $row['customer_FirstName'].' '.$row['customer_LastName']; ?><br>
									...
									<div class="modal-content">
										...
										<div class="modal-body" style="font-size: 14px">
											<form action="" method="post">
                                                ...
                                                <input type="hidden" name="cust_first_name" value="<?php echo $row['customer_FirstName']; ?>">
                                                <input type="hidden" name="cust_email" value="<?php echo $row['customer_email']; ?>">
                        ...
                        <td>
                           <?php
                           $statement1 = $pdo->prepare("SELECT * FROM tbl_order WHERE payment_id=? AND company_id=?");
                           $statement1->execute(array($row['payment_id'],$_SESSION['company']['company_id']));
                        ...
                        <td>
                            ...
                            <?php
                                if($row['payment_status']=='Pending'){
                                    ?>
                                    <a href="order-change-status.php?id=<?php echo $row['pay_id']; ?>&task=Completed" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Mark Complete</a>
                        ...
                        <td>
                            ...
                            if($row['payment_status']=='Completed') {
                                if($row['shipping_status']=='Pending'){
                                    ?>
                                    <a href="shipping-change-status.php?id=<?php echo $row['pay_id']; ?>&task=Completed" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Mark Complete</a>
                                    <?php
                                }
                            }
                            if($row['shipping_status']=='Completed'){
                                $or_id=2;
                                $statement = $pdo->prepare("UPDATE tbl_order SET order_status_id=? WHERE payment_id=?");
                                $statement->execute(array($or_id,$row['payment_id']));
                            
                            }
                            ?>
                        </td>
	                    <td>
                            <a href="#" class="btn btn-danger btn-xs" data-href="order-delete.php?id=<?php echo $row['pay_id']; ?>" data-toggle="modal" data-target="#confirm-delete" style="width:100%;">Delete</a>
